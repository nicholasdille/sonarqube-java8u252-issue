...
if ! test -d docker-maven; then
    ...
    sed -i 's/^FROM openjdk:8-jdk-slim/FROM openjdk:8u242-jdk-slim/' openjdk-8u242-slim/Dockerfile
    cp -r openjdk-8-slim openjdk-8u252-slim
    sed -i 's/^FROM openjdk:8-jdk-slim/FROM openjdk:8u252-jdk-slim/' openjdk-8u252-slim/Dockerfile
    cd ..
fi
...
docker-compose exec --workdir /tmp java8u242 \
    bash -c 'test -d /tmp/sonar-scanning-examples || git clone https://github.com/SonarSource/sonar-scanning-examples'
docker-compose exec --workdir /tmp/sonar-scanning-examples/sonarqube-scanner-maven/maven-basic java8u242 \
...
docker-compose exec --workdir /tmp java8u252 \
    bash -c 'test -d /tmp/sonar-scanning-examples || git clone https://github.com/SonarSource/sonar-scanning-examples'
docker-compose exec --workdir /tmp/sonar-scanning-examples/sonarqube-scanner-maven/maven-basic java8u252 \
    sed -i 's|<maven.compiler.release>11</maven.compiler.release>||' pom.xml
docker-compose exec --workdir /tmp/sonar-scanning-examples/sonarqube-scanner-maven/maven-basic java8u252 \
    mvn clean verify sonar:sonar -Dsonar.host.url=https://${SONARQUBE_DNS_NAME}
